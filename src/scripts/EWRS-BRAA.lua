...
ewrs.allowBogeyDope = false -- Allows pilots to request a bogey dope even with the automated messages running. It will display only the closest threat, and will always reference the players own aircraft.
ewrs.allowFriendlyPicture = true -- Allows pilots to request picture of friendly aircraft
ewrs.maxFriendlyDisplay = 6 -- Limits the amount of friendly aircraft shown on friendly picture
ewrs.restrictMeasurements = true
ewrs.subMenu = false
	...
	local status, result = pcall(function()
		for i = 1, #ewrs.activePlayers do
			...
			if ewrs.builtF10Menus[stringGroupID] == nil then
				...
				if ewrs.subMenu then
					rootPath = missionCommands.addSubMenuForGroup(groupID, "EWRS")
				else
					rootPath = nil
				end
				...
				if not ewrs.restrictMeasurements then
